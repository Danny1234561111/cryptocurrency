            ...
            if (currentRateString.isNotEmpty()) {
                try {
                    ...
                    if (currentRate >= targetRate) {
                        Log.d(TAG, "Target rate reached: $currentRate >= $targetRate")
                        handler.post {
                            Toast.makeText(this@RateCheckService, "$selectedOption3 Достигнули $targetRate$selectedOption2", Toast.LENGTH_SHORT).show()
                        }
                        stopSelf()
                    } else {
                        rateCheckAttempt++
